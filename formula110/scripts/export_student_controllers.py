#!/usr/bin/env python3
"""Export a targeted controller submission with its runtime files."""
...
DEFAULT_ARCHIVE_SUFFIX = "-formula110-student-controllers.zip"
...
def default_archive_name(now: datetime | None = None) -> str:
    """Return the timestamped default filename for a controller export."""
...
def export_controller(controller_module: str, output: Path) -> Path:
    """Write the controller package and dependency metadata to a zip file."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="controller module to grade, such as controllers.race_faster",
    )
    parser.add_argument(
        "--output",
        type=Path,
        help=(
            ...
            "time formatted as yy.mm.dd-hh.mm.ss-formula110-student-controllers.zip)"
        ),
    )
    return parser
...
def main() -> None:
    ...
    try:
        output = export_controller(str(args.controller_module), output_path)
